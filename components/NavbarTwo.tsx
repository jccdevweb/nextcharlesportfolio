  ...
  return (
    <div className='shadow-md w-full fixed top-0 left-0 z-10'>
      <div className=" text-center md:flex items-center justify-between bg-white py-4 md:px-10 px-7">
        ...
        <div onClick={HandleHamburger} className="text-black text-3xl absolute right-8 top-6 cursor-point md:hidden cursor-pointer">
        ...
        <ul className={`md:flex md:items-center md:pb-0 pb-12 absolute md:static bg-white md:z-auto z-[-1] left-0 w-full md:w-auto md:pl-0  transition-all duration-500 ease-in ${showNav ? 'top-15opacity-100' : 'top-[-490px] opacity-0 md:opacity-100'}`
      }>
       
        {
          ...
          Links.map((link)=>(
            <li key={link.id} className="nav-item md:ml-8 text-xl md:my-0 my-7">
              <Link onClick={HandleHamburger} className="nav-item text-gray-800 hover:text-gray-400 duration-500" href={link.link}>{link.name}</Link>
            </li>))
        }
        </ul>
        </div>
